...
class ClientApp(Ice.Application):
    ...
    def run(self, args):
        ...
        self.label = Label(self.window, text="Bienvenido a Authentication :D", font=label_font)
    ...
    def show_authentication_data(self, authentication, my_adapter): 
        ...
        label_font = font.Font(family='Helvetica', size=16, weight='bold')
        label = Label(new_window, text="Authentication", font=label_font)
        label.pack(pady=10)
        window_width = 600
        window_height = 320
    ...
    def login_user(self, authentication):
        new_window = Toplevel(self.window)
        new_window.title("Iniciar Sesión")
        new_window.resizable(False, False) 
        ...
        Button(new_window, text="Autenticar", command=lambda: self.login_user_action(username_entry.get(), password_entry.get(), authentication, success_label)).pack()
    ...
    def show_user_data(self, my_adapter):
        ...
        label_font = font.Font(family='Helvetica', size=16, weight='bold')
        label = Label(new_window, text="User", font=label_font)
        label.pack(pady=10)
        ...
        login_button = Button(new_window, text="Nombres de usuarios logeados", command= lambda: self.get_usernames())
        login_button.pack(pady=5)

        create_user_button = Button(new_window, text="Comprobar si sigue vivo", command=lambda: self.isAlive_user())
        create_user_button.pack(pady=5)

        remove_user_button = Button(new_window, text="Comprobar si sigue vivo después de 2 min", command=lambda: self.isAlive_2min_user())
        remove_user_button.pack(pady=5)
        
        verify_user_button = Button(new_window, text="Renovar validez", command= lambda: self.extend_validity())
        verify_user_button.pack(pady=5)
        
        verify_user_button = Button(new_window, text="Renovar (comprobar fallos)", command= lambda: self.extend_validity_error(my_adapter))
        verify_user_button.pack(pady=5)
    ...
    def isAlive_user(self):
        ...
        Button(new_window, text="Comprobar", command=lambda: self.isAlive_action(username_entry.get(), success_label)).pack()
    ...
    def isAlive_2min_user(self):
        new_window = Toplevel(self.window)
        new_window.title("Usuario Vivo 2 min")
        new_window.resizable(False, False) 
        ...
        Button(new_window, text="Comprobar", command=lambda: self.isAlive_2min_action(username_entry.get(), success_label)).pack()
    ...
    def extend_validity(self):
        new_window = Toplevel(self.window)
        new_window.title("Extender Validez")
        new_window.resizable(False, False) 
